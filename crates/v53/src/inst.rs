...
impl CPU {
    ...
    pub fn instruction (&mut self, op: u8) -> (
        ...
        match op {
            ...
            0x0E => (format!("Push PS to stack"), vec![op], Box::new(push_ps)),

            0x0F => (format!("GROUP3"), vec![op], Box::new(group3_instruction)),
            ...
            0x8C => {
                ...
                (format!("MOV mem, sreg"), vec![op, arg], Box::new(move |cpu: &mut CPU|{
                    ...
                    if mode == 0b11 {
                        let dst = cpu.register_reference_u16(arg & B_MEM);
                        *dst = value;
                        2
                    } else {
            ...
            0x8E => {
                ...
                (format!("MOVW sreg, mem"), vec![op, arg], Box::new(move |cpu: &mut CPU|{
                    ...
                    if mode == 0b11 {
                        ...
                        let dst = cpu.segment_register_reference((arg & B_SREG) >> 3);
                        *dst = src;
                        2
                    } else {
